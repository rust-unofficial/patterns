...
Well-designed Rust FFI follows C API design principles, while compromising the design in Rust as little ass possible. There are three goals with any foreign API:
...
1. Keep the potential for memory unsafety and Rust UB as small as possible.
...
First, the API itself becomes less expressive. With POSIX DBM, there is only one iterator per object, and every call changes its state. This is much more restrictive than iterators in almost any language, even though it is safe. Perhaps with other related objects, whose lifetimes are less heirarchical, this limitation is more of a cost than the safety.
...
* [FFI Error Passing](./ffi-errors.md) explains error handling is with integer codes and sentinel return values (such as null pointers)
